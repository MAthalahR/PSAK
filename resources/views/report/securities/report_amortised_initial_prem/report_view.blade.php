    ...
    <div class="main-content" style="padding-top: 20px;">
        <div class="container mt-5">
            <section class="section">
                <div class="mb-3">
                    <a href="{{ route('amortisedinitialprem.exportPdf',['no_acc' => $reports->first()->no_acc, 'id_pt' => $reports->first()->id_pt])   }}" class="btn btn-danger ">Export to PDF</a>
                    <a href="{{ route('amortisedinitialprem.exportExcel', ['no_acc' => $reports->first()->no_acc, 'id_pt' => $reports->first()->id_pt])   }}" class="btn btn-success ">Export to Excel</a>
                </div>
                        ...
                        <form>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6 row d-flex align-items-center mb-1">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text font-size 12px" class="form-control form-control-sm" value="{{ number_format($reports->first()->outsamt_prem, 0) }}" readonly>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-6 row d-flex align-items-center mb-1">
                                    ...
                                    <div class="col-sm-8">
                                        <input type="text font-size 12px" class="form-control form-control-sm" value="{{ number_format($reports->first()->eircalc_prem * 100, 14) }}%" readonly>
                                    </div>
                                </div>
                            </div>
                    ...
                    <table class="table table-striped table-bordered table-hover table-sm" style="font-size: 12px;">
                        <thead class="thead-dark">
                            <tr>
                                ...
                                <th class="text-center">Effective Interest Base On Effective Yield </th>
                                <th class="text-center">Accrual Coupon</th>
                                <th class="text-center">Amortised At Premium</th>
                                <th class="text-center">Outstanding Amount Initial At Premium</th>
                                <th class="text-center">Cummulative Amortized At Premium</th>
                                <th class="text-center">Unamortized At Premium</th>
                            </tr>
                        </thead>
                        <tbody>
                            @php
                                $unamortisedAtPremium = $reports->first()->atpremium ;
                            @endphp
                            @foreach ($reports as $report)
                            @php
                                $unamortisedAtPremium += $report->amortise_prem;
                            @endphp
                                <tr>
                                    ...
                                    <td class="text-right">{{ number_format($report->outprem, 0) }}</td>
                                    <td class="text-right">{{ number_format($report->accr_prem ?? 0, 0) }}</td>
                                    <td class="text-right">{{ number_format($report->interest, 0) }}</td>
                                    <td class="text-right">{{ number_format($report->amortise_prem, 0) }}</td>
                                    <td class="text-right">{{ number_format($report->outsamt_prem, 0) }}</td>
                                    <td class="text-right">{{ number_format($report->cum_amortiseprem, 0) }}</td>
                                    <td class="text-right">{{ number_format($unamortisedAtPremium ?? 0, 0) }}</td>
                                </tr>
                            @endforeach
                            ...
                            <tr class="table-secondary font-weight-normal">
                                ...
                                <td class="text-right">{{ number_format($reports->sum('outprem'), 0) }}</td>
                                <td class="text-right">{{ number_format($reports->sum('accr_prem'), 0) }}</td>
                                <td class="text-right">{{ number_format($reports->sum('interest'), 0) }}</td>
                                <td class="text-right">{{ number_format($reports->sum('amortise_prem'), 0) }}</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>
