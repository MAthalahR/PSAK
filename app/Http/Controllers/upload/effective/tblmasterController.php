    ...
    public function executeStoredProcedure(Request $request)
{
    try {
        ...
        foreach ($uploadData as $record) {
            try {
                // Pindahkan data dari tabel upload ke tabel master
                $masterData = (array)$record;
                unset($masterData['id']); // Hapus ID jika ada
                
                // Insert ke tabel master
                DB::table('tblmaster_amp')->insert($masterData);
        ...
        // Hapus data dari tabel upload setelah berhasil dipindahkan
        if ($processedCount > 0) {
        ...
        // Prepare response message
        $message = "Berhasil memproses $processedCount data ke tabel master";
        if (!empty($errors)) {
    ...
    public function clear(Request $request)
    {
        try {
            ...
            // Hapus data dari kedua tabel
            $deletedTmp = DB::table('tblmaster_tmp')
                ->where('id_pt', $id_pt)
                ->delete();
        
            $deletedUpload = DB::table('tblmaster_tmp_upload')
                ->where('id_pt', $id_pt)
                ->delete();
        
            $totalDeleted = $deletedTmp + $deletedUpload;
        
            if ($totalDeleted > 0) {
                DB::commit();
                Log::info('Successfully deleted ' . $totalDeleted . ' records');
                return redirect()->back()->with('success', 'Berhasil menghapus ' . $totalDeleted . ' data');
            } else {
                DB::rollBack();
